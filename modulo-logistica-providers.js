...
(function () {
  ...
  const SCHEMA = {
    noventanove: {
      icon: '🚗',
      grupos: [
        { titulo: '🔑 Credenciais 99 Corp API', campos: [
          { key: 'api_key',        label: 'API Key (x-api-key)', secret: true,  hint: 'x-api-key da conta 99' },
          { key: 'employee_id',    label: 'Employee ID',          secret: false, hint: 'colaborador técnico' },
          { key: 'cost_center_id', label: 'Cost Center ID',       secret: false, hint: 'centro de custo' },
          { key: 'project_id',     label: 'Project ID (opcional)',secret: false },
        ]},
        { titulo: '🪝 Webhook (Basic Auth)', campos: [
          { key: 'webhook_username', label: 'Webhook Username', secret: false },
          { key: 'webhook_password', label: 'Webhook Password', secret: true },
        ]},
      ],
    },
    uber: {
  ...
  function ProviderCard({ provider, expandido, onToggle, apiBase, webhookUrlDe, fetchAuth, showToast, onSalvou }) {
    ...
    // Carrega o detalhe na 1ª expansão.
    useEffect(() => {
      ...
      (async () => {
        ...
        try {
          ...
          if (!abortado) showToast('Erro de rede ao carregar provider', 'error');
        } finally {
          if (!abortado) setCarregandoDet(false);
        }
      })();
    ...
    function montarPayload() {
      ...
      schema.grupos.forEach(g => g.campos.forEach(campo => {
        ...
        if (campo.topLevel) {
          // segredo top-level (ex: uber webhook_secret) — só envia se digitou
    ...
    return e('div', {
      ...
      e('div', {
        ...
        e('span', { className: 'text-2xl' }, iconDe(code)),
  ...
  function ProviderForm(p) {
    ...
    const SecretField = (campo) => {
      const setado = campo.topLevel
        ? !!detalhe[campo.key + '_setado']
        : !!form.config[campo.key + '_setado'];
    ...
    return e('div', null,
      ...
      schema.grupos.map((grupo, gi) =>
        e('div', { key: gi, className: 'mb-4' },
          ...
          grupo.campos.map(campo => campo.secret ? SecretField(campo) : Field(campo))
        )
      ),
