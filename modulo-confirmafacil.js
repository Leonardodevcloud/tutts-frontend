...
// modulo-confirmafacil.js
// Duas abas: Config (embarcadores) + NFs Recebidas
// Admin only
// ===============================================================
...
(function() {
  ...
  function ModalEmbarcador({ clienteId, embarcador, onSalvar, onFechar, fetchAuth, API_URL, showToast }) {
    ...
    const Input = ({ label, campo, tipo, placeholder, half }) =>
      h('div', { className: half ? 'col-span-1' : 'col-span-2' },
    ...
    return h('div', {
      ...
      h('div', { className: 'bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto' },
        h('div', { className: 'flex items-center justify-between p-5 border-b border-gray-100' },
          ...
          h('button', { onClick: onFechar, className: 'text-gray-400 hover:text-gray-600 text-xl' }, '×')
        ),
        h('div', { className: 'p-5 grid grid-cols-2 gap-4' },
          Input({ label: 'CNPJ do Embarcador *', campo: 'cnpj_embarcador', half: false, placeholder: '00.000.000/0000-00' }),
          Input({ label: 'Nome do Embarcador', campo: 'nome_embarcador', half: false }),

          h('div', { className: 'col-span-2' },
            h('p', { className: 'text-xs font-semibold text-purple-700 uppercase tracking-wide mb-2 mt-1' },
              '📍 Endereço de Coleta')
          ),
          Input({ label: 'Nome Fantasia (ponto de coleta)', campo: 'coleta_nome_fantasia', half: false }),
          Input({ label: 'Rua / Logradouro', campo: 'coleta_rua', half: false }),
          Input({ label: 'Número', campo: 'coleta_numero', half: true }),
          Input({ label: 'Bairro', campo: 'coleta_bairro', half: true }),
          Input({ label: 'Cidade *', campo: 'coleta_cidade', half: true }),
          Input({ label: 'UF *', campo: 'coleta_uf', half: true, placeholder: 'SP' }),
          Input({ label: 'CEP', campo: 'coleta_cep', half: true }),
          Input({ label: 'Telefone', campo: 'coleta_telefone', half: true }),
          Input({ label: 'Latitude', campo: 'coleta_lat', half: true }),
          Input({ label: 'Longitude', campo: 'coleta_lng', half: true }),
        ),
  ...
  function AbaConfig({ fetchAuth, API_URL, showToast }) {
    ...
    const [modalEmb, setModalEmb] = useState(null); // null | 'novo' | objeto
    const [loading, setLoading] = useState(false);
    const [testando, setTestando] = useState(false);

    // Carregar clientes de solicitação
    useEffect(() => {
    ...
    // Carregar config quando seleciona cliente
    useEffect(() => {
      ...
      fetchAuth(API_URL + '/confirmafacil/embarcadores/' + clienteSel)
        .then(r => r.json())
        .then(d => setEmbarcadores(d.embarcadores || []))
        .catch(() => {});
    }, [clienteSel]);
    ...
    async function salvarConfig() {
      ...
      if (!formConfig.cf_email || !formConfig.cf_senha || !formConfig.cnpj_transportadora) {
        showToast('Email, senha e CNPJ transportadora são obrigatórios', 'error'); return;
      }
      setLoading(true);
      try {
    ...
    async function testar() {
      ...
      try {
        ...
        const d = await r.json();
        if (d.ok) showToast('✅ Credenciais válidas!', 'success');
        else showToast('❌ ' + (d.mensagem || 'Credenciais inválidas'), 'error');
      } catch(_) { showToast('Erro de conexão', 'error'); }
    ...
    async function excluirEmb(id) {
      ...
      try {
        await fetchAuth(API_URL + '/confirmafacil/embarcadores/' + id, { method: 'DELETE' });
          ...
          h('div', { className: 'col-span-2 flex items-center gap-6' },
            ...
            h('label', { className: 'flex items-center gap-2 text-sm cursor-pointer' },
              h('input', { type: 'checkbox', checked: formConfig.polling_ativo,
                onChange: e => setF('polling_ativo', e.target.checked),
                className: 'accent-purple-600' }),
              'Polling automático'
            )
          )
      ...
      clienteSel && config && h('div', { className: 'bg-white rounded-2xl border border-gray-100 shadow-sm p-5' },
        h('div', { className: 'flex items-center justify-between mb-4' },
          ...
          h('button', {
            ...
            className: 'flex items-center gap-1.5 px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-xl hover:bg-purple-700',
          }, '+ Novo Embarcador')
        ),

        embarcadores.length === 0
          ? h('div', { className: 'text-center py-8 text-gray-400 text-sm' },
              '📭 Nenhum embarcador configurado. Adicione o primeiro acima.')
          : h('div', { className: 'space-y-3' },
              embarcadores.map(emb =>
                h('div', { key: emb.id, className: 'border border-gray-100 rounded-xl p-4 hover:border-purple-200 transition-colors' },
                  h('div', { className: 'flex items-start justify-between gap-4' },
                    h('div', { className: 'flex-1 min-w-0' },
                      ...
                      h('div', { className: 'flex items-center gap-1.5 mt-2' },
                        h('span', { className: 'text-xs text-purple-600' }, '📍'),
                    ...
                    h('div', { className: 'flex gap-2 shrink-0' },
                      h('button', {
                        onClick: () => setModalEmb(emb),
                        className: 'text-xs px-3 py-1.5 border border-gray-200 rounded-lg hover:border-purple-300 text-gray-600',
                      }, '✏️ Editar'),
                      h('button', {
                        onClick: () => excluirEmb(emb.id),
  ...
  function AbaNFs({ fetchAuth, API_URL, showToast }) {
    ...
    async function carregarVinculos() {
      setLoading(true);
      try {
        const r = await fetchAuth(API_URL + '/confirmafacil/nfs/' + clienteSel);
        const d = await r.json();
    ...
    async function forcarPolling() {
      setForcando(true);
      try {
        const r = await fetchAuth(API_URL + '/confirmafacil/polling/forcar', { method: 'POST' });
        const d = await r.json();
        showToast(d.mensagem || '✅ Polling iniciado', 'success');
        setTimeout(carregarVinculos, 3000);
      } catch(_) { showToast('Erro', 'error'); }
      finally { setForcando(false); }
    }

    const filtrados = vinculos.filter(v =>
      !busca || v.numero_nf?.toLowerCase().includes(busca.toLowerCase()) ||
      v.cnpj_embarcador?.includes(busca) || String(v.id_embarque).includes(busca)
    );
    ...
    return h('div', { className: 'space-y-4' },
      ...
      h('div', { className: 'flex flex-wrap items-center gap-3' },
        h('select', {
          value: clienteSel,
          onChange: e => { setClienteSel(e.target.value); setPagina(0); },
        ...
        clienteSel && h('input', {
          type: 'text', placeholder: '🔍 Buscar NF, CNPJ, ID...',
          value: busca, onChange: e => { setBusca(e.target.value); setPagina(0); },
          className: 'border border-gray-200 rounded-xl px-3 py-2.5 text-sm w-56 focus:outline-none focus:ring-2 focus:ring-purple-400',
        }),
        clienteSel && h('button', {
          onClick: carregarVinculos,
          className: 'px-4 py-2.5 border border-gray-200 text-sm rounded-xl hover:border-purple-300',
        }, '🔄 Atualizar'),
        clienteSel && h('button', {
          onClick: forcarPolling, disabled: forcando,
          className: 'px-4 py-2.5 bg-purple-600 text-white text-sm rounded-xl hover:bg-purple-700 disabled:opacity-50',
        }, forcando ? '⏳ Buscando...' : '⚡ Forçar Polling')
      ),

      // Tabela
      clienteSel && h('div', { className: 'bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden' },
        loading
          ? h('div', { className: 'flex items-center justify-center h-40 text-gray-400 text-sm' }, '⏳ Carregando...')
          : filtrados.length === 0
            ? h('div', { className: 'flex flex-col items-center justify-center h-40 text-gray-400' },
                h('span', { className: 'text-3xl mb-2' }, '📭'),
                h('p', { className: 'text-sm' }, 'Nenhuma NF recebida ainda'),
                h('p', { className: 'text-xs mt-1' }, 'Use "Forçar Polling" para buscar agora')
              )
            : h('div', null,
                  ...
                  h('table', { className: 'w-full text-sm' },
                    h('thead', null,
                      h('tr', { className: 'bg-gray-50 border-b border-gray-100' },
                        ['ID Embarque','NF','Série','CNPJ Emb.','Corrida','Status','Criado em'].map(col =>
                    ...
                    h('tbody', null,
                      paginados.map((v, i) =>
                        h('tr', { key: v.id, className: i % 2 === 0 ? 'hover:bg-gray-50' : 'bg-gray-50/50 hover:bg-gray-50' },
                          h('td', { className: 'px-4 py-3 font-mono text-xs text-gray-600' }, v.id_embarque),
                          h('td', { className: 'px-4 py-3 font-medium text-gray-800' }, v.numero_nf || '—'),
                          h('td', { className: 'px-4 py-3 text-gray-600' }, v.serie_nf || '—'),
                          h('td', { className: 'px-4 py-3 text-xs text-gray-500 font-mono' }, fmtCNPJ(v.cnpj_embarcador)),
                          ...
                          h('td', { className: 'px-4 py-3' }, h(Badge, { status: v.status || 'enviado' })),
                          h('td', { className: 'px-4 py-3 text-xs text-gray-500' }, fmtData(v.criado_em))
                        )
                      )
                    )
                  )
                ...
                pagTotal > 1 && h('div', { className: 'flex items-center justify-between px-4 py-3 border-t border-gray-100' },
                  ...
                  h('div', { className: 'flex gap-2' },
                    ...
                    h('button', {
                      onClick: () => setPagina(p => Math.min(pagTotal-1, p+1)),
                      disabled: pagina >= pagTotal-1,
                      className: 'px-3 py-1.5 text-xs border border-gray-200 rounded-lg disabled:opacity-40',
                    }, 'Próx →')
                  )
                )
